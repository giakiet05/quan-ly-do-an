# Script create classroom via API
...
$body = @{
    name = "Do an Web Development - K18"
    description = "Full-stack web development course with React and MongoDB"
    semester = "HK1"
    year = 2024
    auto_approve = $true
    max_students = 100
...
try {
    ...
    Write-Host "`nJoining classroom as student..." -ForegroundColor Yellow
    $joinBody = @{
        invitation_code = $response.data.invitation_code
    } | ConvertTo-Json
    
    $joinResponse = Invoke-RestMethod -Uri "http://localhost:8080/api/classrooms/join" -Method POST -Headers $headers -Body $joinBody
    Write-Host "Joined successfully!" -ForegroundColor Green
