<script lang="ts">
  ...
  const handleInput = (index: number, e: Event) => {
    ...
    if (val && index < 5) { // index < 5 vì mảng có 6 phần tử (0-5)
...
<div class="otp-container">
  <div class="auth-header">
     <div class="icon-wrapper">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="shield-icon">
            <path fill-rule="evenodd" d="M12.516 2.17a.75.75 0 0 0-1.032 0 11.209 11.209 0 0 1-7.877 3.08.75.75 0 0 0-.722.515A12.74 12.74 0 0 0 2.25 9.75c0 5.942 4.064 10.933 9.563 12.348a.749.749 0 0 0 .374 0c5.499-1.415 9.563-6.406 9.563-12.348 0-1.352-.272-2.636-.759-3.803a.75.75 0 0 0-.722-.515 11.208 11.208 0 0 1-7.877-3.08ZM12 13.5a1.5 1.5 0 1 0 0-3 1.5 1.5 0 0 0 0 3Z" clip-rule="evenodd" />
        </svg>          
     </div>
  ...
  <div class="timer-wrapper">
    {#if timeLeft > 0}
        <span class="timer-text">Mã hết hạn sau: <span class="time">{formattedTime}</span></span>
    {:else}
  ...
  <div class="footer-text">
    ...
    <button class="resend-btn" on:click={handleResend} disabled={timeLeft > 0 || loading}>
      Gửi lại
    </button>
  </div>
</div>
...
<style>
  ...
  .shield-icon { width: 48px; height: 48px; color: var(--primary-blue); }
  ...
  .time { color: #ef4444; font-weight: 600; }
