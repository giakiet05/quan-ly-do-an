<script lang="ts">
  ...
  const clearError = () => { if(error) error = ""; }
  ...
  {#if step === 'email'}
    <div class="auth-header">
        <div class="icon-wrapper">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="key-icon">
                <path fill-rule="evenodd" d="M15.75 1.5a6.75 6.75 0 0 0-6.651 7.906c-1.067.322-2.02.854-2.841 1.551l-5.819 4.946a.75.75 0 0 0-.066 1.056l2.499 2.996a.75.75 0 0 0 .736.257l2.846-.711a.75.75 0 0 0 .524-.483l.865-2.597.575-.575a6.75 6.75 0 1 0 7.332-14.343ZM15.75 3a5.25 5.25 0 1 1 0 10.5 5.25 5.25 0 0 1 0-10.5Z" clip-rule="evenodd" />
            </svg>              
        </div>
      ...
      <form on:submit|preventDefault={handleResetPassword} class="auth-form">
        ...
        <div class="form-group">
            ...
            <div class="input-wrapper-with-icon">
                ...
                <div class="toggle-password-wrapper">
                    <Button variant="icon" type="button" onclick={togglePassword} tabindex="-1">
                        {#if showPassword}
        ...
        <div class="form-group">
            ...
            <div class="input-wrapper-with-icon">
                ...
                />
                <div class="toggle-password-wrapper">
                    <Button variant="icon" type="button" onclick={toggleConfirmPassword} tabindex="-1">
...
<style>
  ...
  .auth-header { margin-bottom: 32px; text-align: center; } /* Căn giữa cho đẹp với form này */
  .icon-wrapper { display: inline-flex; justify-content: center; align-items: center; margin-bottom: 16px; background: #eff6ff; padding: 12px; border-radius: 50%; }
  .key-icon { width: 32px; height: 32px; color: var(--primary-blue); }
  .title { font-size: 26px; font-weight: 700; color: var(--text-dark); margin: 0 0 8px 0; }
  .subtitle { font-size: 15px; color: var(--text-gray); margin: 0; line-height: 1.5; }

  .auth-form { text-align: left; }
  .form-group { margin-bottom: 24px; }
  label { display: block; font-size: 15px; font-weight: 600; color: var(--text-dark); margin-bottom: 8px; }
  ...
  .back-login { text-align: center; margin-top: 32px; }
  .back-link {
    display: inline-flex; align-items: center; justify-content: center;
    color: var(--text-gray); text-decoration: none; font-weight: 600; font-size: 15px; transition: color 0.2s;
  }
  .back-link:hover { color: var(--text-dark); }
  .arrow-icon { width: 18px; height: 18px; margin-right: 6px; }

  .input-wrapper-with-icon { position: relative; }
  input.has-icon-right { padding-right: 48px; }
  .toggle-password-wrapper { position: absolute; right: 8px; top: 50%; transform: translateY(-50%); z-index: 10; }
  .eye-icon { width: 20px; height: 20px; }

  .error-alert { background-color: #fee2e2; color: #991b1b; padding: 12px; border-radius: 8px; margin-bottom: 20px; font-size: 14px; }
  
  :global(.w-full) { width: 100% !important; display: flex !important; }
  :global(.mt-4) { margin-top: 16px !important; }
  ...
  .link-text-small {
      ...
      font-size: 14px; text-decoration: underline; cursor: pointer; padding: 5px;
  }
  .link-text-small:hover { color: var(--text-dark); }
</style>